...
# Als de Modbus-Tool een andere slave ID of coil adres aangeeft, pas deze hier aan:
# Bijv. MODBUS_UNIT = 2, COIL_ADDRESS = 1
...
client = ModbusSerialClient(
    ...
    baudrate=BAUDRATE,
    timeout=3,
    parity='E',  # Gewijzigd naar Even pariteit, zoals in de Modbus-Tool
    stopbits=1,
    bytesize=8
)
...
def set_relay_state(state):
    ...
    if fallback_mode:
        ...
        try:
            client.unit_id = MODBUS_UNIT  # Stel slave ID in
            result = client.write_coil(current_coil, state)  # Geen unit/slave argument
            if result.isError():
