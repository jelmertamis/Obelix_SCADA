...
<body>
  ...
  <script>
    ...
    // Registreer raw_value handler één keer
    socket.on('raw_value', d => {
      ...
      // Update specifiek veld als punt is gespecificeerd
      if (d.point && d.raw !== null) {
        document.getElementById(`raw${d.point}-${key}`).value = d.raw;
      }
    });
    ...
    function fetchRaw(unit, channel, point) {
      socket.emit('get_raw', {unit_index: unit, channel: channel, point: point});
    }
